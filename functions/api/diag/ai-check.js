// Temporary diagnostic route -- no payment required, tests the actual AI
// binding and storage machinery in isolation. NOT for production; remove
// after verifying.
export async function onRequestGet(context) {
  const { env } = context;
  const results = {};

  // Test A: Gemma structured JSON call
  try {
  ...
  // Test B: toMarkdown on a tiny inline text buffer
  try {
  ...
  // Test C: Whisper on a minimal synthetic silent WAV (no external fetch needed)
  try {
    const sampleRate = 8000, numSamples = 800; // 0.1s of silence
